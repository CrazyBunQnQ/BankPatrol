                ...
                <c:forEach items="${data.pageData}" var="dept" >
                    ...
                    <td>
                    	<div class="handle">
                            ...
                            <a href="../dept/dept2Update.do?deptId=${dept.id}"><span class="handle-icon fix" ></span></a>
                        </div>
                    </td>
                </tr>
                </c:forEach>
 ...
 <p align="right">
	<font color="black">共</font><font color="black">${data.totalRecord}</font>
		...
		<c:if test="${data.curPage!=data.totalPage }">
			<a href="../dept/deptList.do?curpage=${data.curPage+1 }&loginId=${log}&userName=${un}">下一页</a>
			<a href="../dept/deptList.do?curpage=${data.totalPage }">尾 页</a>
		</c:if>
		第<input id="pagebox" type="text" size="4">页
		<a onclick="jump('../dept/deptList.do?curpage=','${data.totalPage }');" href="javascript:;">跳转</a>
		   </p></div>
</body>
<script>
